...
def extract_keywords(papers: List[Paper], top_n: int = 15) -> List[str]:
    """Extract most frequent meaningful words from abstracts."""
    ...
    for subtopic, keywords in SUBTOPIC_PROBES:
        ...
        if hits == 0:
            gaps.append(ResearchGap(
                area=subtopic,
                description=f"No papers in the retrieved set address {subtopic} in this context.",
                confidence=0.9,
            ))
        elif hits == 1:
            gaps.append(ResearchGap(
                area=subtopic,
                description=f"Very limited research found on {subtopic} — only a single mention detected.",
                confidence=0.6,
            ))
...
def generate_future_directions(
    ...
    for gap in gaps[:3]:
        directions.append(
            f"Investigate {gap.area} in the context of {', '.join(keywords[:2]) if keywords else 'this domain'}."
        )

    growing = [t for t in trends if t.direction == "growing"]
    if growing:
        directions.append(
            f"Extend recent momentum in this field — {growing[0].evidence}"
        )

    saturated = [t for t in trends if t.direction == "saturated"]
    if saturated:
        directions.append(
            "Consider applying techniques from adjacent fields to bring fresh perspectives."
        )
