...
# ─────────────────────────────────────────
WEIGHT_RELEVANCE = 0.55    # Semantic similarity (most important)
WEIGHT_CITATION  = 0.25    # Impact / authority
WEIGHT_RECENCY   = 0.20    # How recent (newer = better)
...
def compute_final_score(
    ...
    """
    r = max(0.0, min(1.0, relevance))          # cosine sim already in [0,1] for normalized
...
def rank_papers(
    ...
    for paper in papers:
        ...
        final = compute_final_score(relevance, paper.citation_count, paper.year or 2015)
